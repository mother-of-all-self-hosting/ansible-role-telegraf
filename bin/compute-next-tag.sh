...
#
# Tags look like `v<Telegraf version>-<release>`, which is what this repository
# has always used (`v1.39.3-0`, `v1.39.1-1`, …):
#
# - if defaults/main.yml points at a Telegraf version that has never been
#   released, the release counter restarts at 0 (`v1.40.0-0`)
# - otherwise the counter is incremented (`v1.39.3-1`), but only if something
...
# Anchored on `telegraf_version:` so that neither `telegraf_container_image_tag:`
# (which is derived from it) nor a commented-out line can be mistaken for it.
# This is the same literal that the `# renovate:` annotation above it applies to.
version="$(sed -nE 's|^telegraf_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Telegraf version from $defaults_path"
	exit 1
fi

# The version value in defaults/main.yml carries no leading `v` (e.g. `1.39.3`)
